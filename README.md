...
- Продуманная админ-панель `/admin`:
  - dashboard со счетчиками пользователей и заявок;
  - фильтры по статусам: новые, одобренные, отклоненные, повторные, заблокированные;
  - пагинация списков заявок;
  - поиск по BingX UID, Telegram ID, username или номеру заявки;
  - карточка заявки с UID, username, датой, модератором, причиной и фото;
  - решения: одобрить, отклонить, запросить повторно, заблокировать;
  - обязательный выбор причины для отказа, повторной отправки и блокировки.
